...
interface AdminCertificationsEditorProps {
  isAdminMode: boolean;
...
export const AdminCertificationsEditor = ({ isAdminMode, certifications, onCertificationsUpdate, sectionType }: AdminCertificationsEditorProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [formData, setFormData] = useState<Certification[]>(certifications);
  const { toast } = useToast();

  const handleSave = () => {
    onCertificationsUpdate(formData);
    setIsOpen(false);
    toast({
  ...
  const updateCertification = (id: number, field: keyof Certification, value: any) => {
    setFormData(formData.map(c => c.id === id ? { ...c, [field]: value } : c));
  };

  const moveCertification = (id: number, direction: 'up' | 'down') => {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          ...
          Edit {sectionType === 'international' ? 'International Certifications' : 'Certifications'}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit {sectionType === 'international' ? 'International Certifications' : 'Certifications'} Section</DialogTitle>
        </DialogHeader>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        ...
                        reader.onload = (event) => {
                          updateCertification(cert.id, 'image', event.target?.result as string);
                        };
